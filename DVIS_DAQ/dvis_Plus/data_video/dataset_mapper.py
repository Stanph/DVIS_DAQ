...
from .datasets.ytvis import COCO_TO_YTVIS_2019, COCO_TO_YTVIS_2021, COCO_TO_OVIS, COCO_TO_PRW, COCO_TO_YTVIS_2019_PERSON
...
class YTVISPERSONDatasetMapper:
    """
    A callable which takes a dataset dict in YouTube-VIS Dataset format,
    and map it into a format used by the model.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if not is_tgt:
            self.src_metadata = MetadataCatalog.get(src_dataset_name)
            self.tgt_metadata = MetadataCatalog.get(tgt_dataset_name)
            if tgt_dataset_name.startswith("ytvis_2019_person"):
                src2tgt = COCO_TO_YTVIS_2019_PERSON
            elif tgt_dataset_name.startswith("ytvis_2019"):
