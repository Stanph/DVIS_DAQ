...
from dvis_Plus.meta_architecture import MinVIS, VideoMaskFormer_frame
from .matcher import FrameMatcher, NewInsHungarianMatcher
from .criterion import DAQCriterion,DAQCriterion_cl
...
class DVIS_DAQ_online(MinVIS):
    ...
    def __init__(
        ...
        self.task = task
        assert self.task in ['prwvis', 'vis', 'vss', 'vps', 'vos'], "Only support vis, vss and vps !"
        inference_dict = {
...
@META_ARCH_REGISTRY.register()
class DVIS_DAQ_online_motion(VideoMaskFormer_frame):
    @configurable
    def __init__(
            self,
            *,
...
@META_ARCH_REGISTRY.register()
class DVIS_DAQ_online_cl(MinVIS):
    @configurable
    def __init__(
            self,
            *,
    ...
    def from_config(cls, cfg):
        ...
        tracker = VideoInstanceCutter(
            hidden_dim=hidden_channel, #ph cfg.MODEL.MASK_FORMER.HIDDEN_DIM
            feedforward_dim=cfg.MODEL.MASK_FORMER.DIM_FEEDFORWARD,
            num_head=cfg.MODEL.MASK_FORMER.NHEADS,
            decoder_layer_num=cfg.MODEL.TRACKER.DECODER_LAYERS,
            mask_dim=cfg.MODEL.MASK_FORMER.HIDDEN_DIM,
    ...
    def forward(self, batched_inputs):
        ...
        losses,reference_match_result = self.daq_criterion(outputs, video_targets)
        ...
        slot_losses,_ = self.daq_criterion(slot_outputs, video_targets[1:])
        new_slot_losses = dict()
        for k in list(slot_losses.keys()):
            if k in self.daq_criterion.weight_dict:
                new_slot_losses["slot_" + k] = slot_losses[k] * self.daq_criterion.weight_dict[k]
